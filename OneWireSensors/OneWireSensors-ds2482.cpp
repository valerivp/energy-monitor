...
/*	static bool searchStarted = false;
	if (!searchStarted) {
		...
		ds2482.wireReset();
		ds2482.wireSkip();
		ds2482.wireWriteByte(OneWireCommands::STARTCONVO);        // start conversion, with parasite power on at the end 
		return;
	}
*/	if (!ds2482.wireSearch(addr)) {
		//searchStarted = false;
		ds2482.wireResetSearch();
		ds2482.wireReset();
		ds2482.wireSkip();
	...
	ds2482.wireReset();
	ds2482.wireSelect(addr);
	ds2482.wireWriteByte(OneWireCommands::READSCRATCH);
	uint8_t databuf[9];
	for (int8_t i = 0; i < sizeof(databuf); i++) {           // we need 9 bytes
		databuf[i] = ds2482.wireReadByte();
	}
	if (ds2482.crc8(databuf, 8) != databuf[8]) {
		String tmp;
